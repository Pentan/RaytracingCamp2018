...
public class ImageWriter {
    // write as BMP
    @discardableResult
    static public func writeBMP(filepath:String, width:Int32, height:Int32, data:Array<Vector3>, gamma:Double = 2.2) -> Bool {
        ...
        if let f = fopen(filepath, "wb") {
            ...
            for rgb in data {
                // write as BGR
                fputc(packU8(rgb.z), f);
                fputc(packU8(rgb.y), f);
                fputc(packU8(rgb.x), f);
            }
